...
class QuestionService:
    ...
    async def add_questions(self, question_data: QuestionCreateRequest, quiz_id: UUID) -> dict:
    ...
    async def get_questions(self,quiz_id:UUID,x_language_code):
        async with self._uow as uow:
            try:
                questions_list = await uow.question_repo.get_list_by_quiz_id(quiz_id, x_language_code)
                res = [
                    {
