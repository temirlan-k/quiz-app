...
class AnswerService:
    ...
    async def answer_question(self, question_id: UUID, user_id: UUID, answer_request: dict, language_code: str):
        async with self._uow as uow:
            try:
                ...
                await self._check_repeat_attempts(uow, question_l, user_id, answer_request)
                
                checker = AnswerCheckerFactory.get_checker(question_l.question.question_type)
                is_correct = checker.check_answer(question_l, answer_request.get('answer_content'))
                ...
                if not is_correct:
                    provider = FeedbackProviderFactory.get_provider(question_l.question.question_type)
    ...
    async def _check_repeat_attempts(self, uow: UnitOfWork, question_l, user_id, answer_request):
