...
class UserQuizSessionService:
    ...
    async def complete_quiz_session(self, user_id: UUID, session_id: UUID) -> dict:
        async with self._uow as uow:
            try:
                ...
                new_correct_question_count = await self._calculate_correct_questions(uow,session_id,user_id)
    ...
    async def _calculate_correct_questions(self,uow: UnitOfWork,session_id:UUID,user_id:UUID):
